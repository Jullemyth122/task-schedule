...
const BoardSelect = () => {
    ...
    const handleCreateTask = async (category) => {
        if (!newTask[category]?.trim() || !selectedBoard) return;

        const updatedTaskList = selectedBoard.taskList.map((task) => {
            const key = Object.keys(task)[0]; 
            if (key === category) {
                return { [key]: [...task[key], newTask[category]] }; 
            }
            return task;
        });
        ...
        try {
            ...
            // Use board.id instead of board.boardTitle in the comparison
            ...
            setActiveCategory({}); // Reset to an object instead of null
        } catch (error) {
        ...
        <div className='board-select'>
            ...
            <div className="list-of-tasks flex items-start justify-start gap-2 flex-wrap">
                {selectedBoard && selectedBoard.taskList.map((task, index) => { 
                    ...
                    const taskItems = task[category] || [];

                    return (
                        <div className="task" key={index}>
                            <div className="td-1">
                                <input type="text" defaultValue={Object.keys(task)[0]} />
                            ...
                            { 
                                !activeCategory[category] &&  // 🟢 Now checks per category
                                <div className="card-btn" onClick={() => toggleCategory(category)}>
                            ...
                            {  activeCategory[category] && 
                                <div className="rd-cd-btn">
                                    ...
                                    <div className="btn-create-cd">
                                        <div className="btn" onClick={() => handleCreateTask(category)}>
                                        Add Card
                                        </div>
                                        <h5 className="cursor-pointer hover:bg-gray-400" 
                                            onClick={() => toggleCategory(category)} // 🟢 Close input when clicking "X"
                                        > 
                                            X 
                                        </h5>
                                    </div>
