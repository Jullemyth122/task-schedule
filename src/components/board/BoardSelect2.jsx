    ...
    return (
        <div className='board-select'>
            ...
            <div className="list-of-tasks flex items-start justify-start gap-2 flex-wrap">
                {selectedBoard && selectedBoard?.taskList.map((task, index) => { 
                    ...
                    return (
                        <div className="task" key={index}>
                            <div className="td-1">
                                <input type="text" defaultValue={categoryTitle} />
                            ...
                            <div className="card-sect grid grid-cols-1 gap-2" >
                                {taskItems.map((taskItem, i) => {
                                    return (
                                        <div className="expand false" key={i}>

                                            {/* 
                                                If Input is click from a certain taskItems then links and taggings will pop up and expand false turn into expand true
                                            */}

                                            <input type="text" key={i} placeholder={`${taskItem}`}  />

                                            {/* { accBST.islinks && inputClickItem
                                                <div className="links">
                                                    { task.links.map }
                                                </div>
                                            }

                                            { accBST.istagging && inputClickItem
                            ...
                            { 
                                !activeCategory[categoryTitle] &&  // checks per categoryTitle
                                <div className="card-btn" onClick={() => toggleCategory(categoryTitle)}>
                            ...
                            {  activeCategory[categoryTitle] && 
                                <div className="rd-cd-btn">
                                    <input
                                        ...
                                        onChange={(e) => setNewTask(prev => ({ ...prev, [categoryTitle]: e.target.value }))}
                                    />
                                    <div className="btn-create-cd">
                                        <div className="btn" onClick={() => handleCreateTask(categoryTitle)}>
                                        Add Card
                                        </div>
                                        <h5 className="cursor-pointer hover:bg-gray-400" 
                                            onClick={() => toggleCategory(categoryTitle)} // close input when clicking "X"
                                        > 
                                            X 
                                        </h5>
                                    </div>
